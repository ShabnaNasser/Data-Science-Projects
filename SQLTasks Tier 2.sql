...
SELECT bf.name, concat(firstname,' ',surname) AS Membername 
...
SELECT DISTINCT f.name, concat(m.firstname,' ',m.surname) AS membername, f.membercost,f.guestcost 
...
WHERE b.starttime = '2012-09-14%' AND (f.membercost>30 OR f.guestcost>30) 
...
SELECT bf.membercost, bf.guestcost, bf.name, concat(m.firstname,' ',m.surname) AS membername 
